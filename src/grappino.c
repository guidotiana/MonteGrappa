...
int main(int argc, char *argv[])
{
	...
	struct rot_input_s *rotamers;
	struct s_potential *u;
	double **cm;
	FILE *fin;

	if (argc != 2) { fprintf(stderr,"usage: grappino file.in\n"); exit(1); }
	...
	npdb = ReadPDB(pdb,p->pdbfile,p->hydrogens,&nchain,&nbackmax,p);			//npdb is the total number of atoms
	...
	// global energy terms
	if (p->e_homo<-EPSILON || p->e_homo> EPSILON) { u->g_ehomo = p->e_homo; u->g_rhomo = p->r_homo; }

	if (p->go_dih) Go_Dihedrals(p,polymer,nchain,u->dih01,u->dih03,u);
	if (p->go_ang) Go_Angles(p,polymer,nchain,u->ang0,u);
	...
	if(strcmp(p->op_file,"")) PrintOpGoFile(p->op_file,cm,polymer,nchain,p->op_kind);
...
void Parse(FILE *fp, struct s_parms *p)
{
	...
	strcpy(p->cntfile,"");
	p->n_back_a = 3;
	strcpy(p->back_a[0],"N"); strcpy(p->back_a[1],"CA"); strcpy(p->back_a[2],"C");
		...
		if (!strncmp(aux,"backbone_atoms",14))
		  {
			  ...
			  ReadParD(aux,"backbone_atoms",&(p->n_back_a));
					...
					if ( sscanf(aux,"backbone_atoms %*d %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s",
					  p->back_a[0],p->back_a[1],p->back_a[2],p->back_a[3],p->back_a[4],p->back_a[5],p->back_a[6],p->back_a[7],
					  p->back_a[8],p->back_a[9],p->back_a[10],p->back_a[11],p->back_a[12],p->back_a[13],p->back_a[14],p->back_a[15],
					  p->back_a[16],p->back_a[17],p->back_a[18],p->back_a[19])<1) Error("Cannot read backbone_atoms in inputfile");
					for(i=0;i<p->n_back_a;i++) p->move_a[i]=1;   		//backbone atoms manually entered are unlocked by default, one must specify if they need to be locked
		  }
				...
				if (!strncmp(aux,"locked_atoms",12))
				{
					...
					if ( sscanf(aux,"locked_atoms %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s",
						locked_a[0],locked_a[1],locked_a[2],locked_a[3],locked_a[4],locked_a[5],locked_a[6],locked_a[7],
						locked_a[8],locked_a[9],locked_a[10],locked_a[11],locked_a[12],locked_a[13],locked_a[14],locked_a[15],
						locked_a[16],locked_a[17],locked_a[18],locked_a[19])>=1)
					{
			...
			{
			p->splice=1;
			if ( sscanf(aux,"splice %lf %lf",&(p->kr_splice), &(p->ke_splice)) < 1 ) Error("Cannot read splice option in inputfile (splice <double> <double>)");
			}
	...
	for (j=0;j<(p->n_back_a);j++) sum += p->move_a[j];
	 ...
	 if(p->splice)
	 {
		 ...
		 if(p->kr_splice>1) Error("in spice option kr_splice can't be bigger than 1\n");
