...
double TotalEnergy(struct s_polymer *p, struct s_potential *u, struct s_mc_parms *parms, int npol, int update, int nosidechains, int debug, int iproc)
{
      ...
      if (parms->noangpot && parms->nodihpot) return etot;
      for (ci=0;ci<npol;ci++)           
            for (i=0;i<(p+ci)->nback;i++)
            {
...
double EnergyMonomer(struct s_polymer *p, struct s_potential *u, int i, int ci, int npol, int update, int shell, int nosidechains,int disentangle,int hb)
{
	...
	else
	{
		...
		{
			j = (((p->back)+i)->shell)[k];									// what is the monomer in the shell
			cj = (((p->back)+i)->shell_p)[k];								// ... what polymers it belongs to
			tooclose = 0;
			if (ci == cj && Abs(i-j) <= u->g_imin) tooclose = 1;
			e = EnergyPair(p,u,i,j,ci,cj,update,nosidechains,disentangle,tooclose,hb);
			if (e>=LARGE && !disentangle) return LARGE;
...
void PrintContacts(FILE *fp, struct s_polymer *p, int step)
{
	...
	{
		fprintf(fp,"Back = %d\n\tside =",i);
		for (j=0;j<((p->back)+i)->nside;j++)
			fprintf(fp,"%d ",((((p->back)+i)->side)+j)->ia);
		fprintf(fp,"\nContacts = ");
		for (j=0;j<((p->back)+i)->ncontacts;j++)
			fprintf(fp,"%d ",(((p->back)+i)->contacts)[j]);
		fprintf(fp,"\n");
		for (j=0;j<((p->back)+i)->ncontacts;j++)
			fprintf(fp,"%lf ",(((p->back)+i)->e)[j]);
		fprintf(fp,"\n");
	}
}
			...
			for (i=0;i<p->nback;i++)
				for (j=0;j<p->nback;j++)
					if (ci!=cj || i<j)
					{
						...
						{
								r2 = Dist2( (((p+cj)->back)+j)->pos, (((((p+ci)->back)+i)->side)+is)->pos );
								if ( r2 < parms->r2shell ) AddShell(p,j,i,cj,ci);			// add to shell
						}
					}
...
int CheckOverlaps(struct s_polymer *p, struct s_potential *u, struct s_mc_parms *parms, int npol, int nosidechains, int pr, FILE *fproc)
{
      ...
      if (pr) fprintf(fproc,"Checking overlaps... \n");
                                    ...
                                    for (js=0;js< (((p+cj)->back)+j)->nside;js++)
                                    {
                                          ...
                                          for (is=0;is< (((p+ci)->back)+i)->nside;is++)
                                          {
                                                ...
                                                if ( r2 < r02 )
                                                {
                                                      ...
                                                      o=1;
                                                      if (pr) fprintf(fproc,"WARNING: overlap between sidechain %d of backbone %d (chain %d, type %d, atom %s, id %d of amino acid %s %d) and sidechain %d of backbone %d (chain %d, type %d, atom %s, id %d of amino acid %s %d): d=%lf d0=%lf\n",
