...
int main(int argc, char *argv[])
{
	...
	#ifdef ACTIVE_MPI
	parms=send_parms(my_rank,nprocs,Parms_mpi,parms,astatus);
	if (strcmp(parms->fnproc,""))
      	...
      	if(parms->debug>1) 
		fprintf(fproc, "iproc=%d\tnrun=%d\tnstep=%d\tnpol=%d\ttemp=%f\tnstep_exchange=%d\n", my_rank, parms->nrun, parms->nstep, parms->npol, parms->T[my_rank],parms->nstep_exchange);
      	#else
      	fproc = stderr;
      	#endif
	...
	ReadPolymer(polname,startp,parms->flog,parms->npol, parms->debug, &nat, &ntypes);
        ...
        for (i=0;i<parms->npol;i++)
	{
        	send_struct(&((startp+i)->nback), my_rank, nprocs, &nat, &ntypes, astatus);
        	startp = send_pol(my_rank, nprocs, (startp+i)->nback, Back_mpi, Side_mpi, Rot_mpi, startp, astatus, i, parms->nshell, parms->nosidechains);
      	}
	if(parms->debug>1) fprintf(fproc,"iproc=%d\tnat=%d\tntypes=%d\tnback=%d\n",my_rank, nat, ntypes, startp->nback);
	...
	u = send_pot(nat,ntypes,parms->noangpot,parms->nodihpot,parms->hb, my_rank, nprocs, Pot_mpi, u, astatus);
...
//                        fprintf(stderr,"\n calcolo a_next = %lf",(((startp+ci)->back)+i)->a_next );
	...
	// open energy file
		...
		Do_MC(polymer,fragment,replica,startp,u,parms,ftrj,fe,oldp,fproc,my_rank,irun,Back_mpi,Side_mpi,Rot_mpi,astatus);
				...
				OP_SamplePotential(polymer,parms,ntypes,u->e,irun,ematold,fp);
	...
	free(parms);
      ...
      MPI_Type_free(&Parms_mpi);
      MPI_Type_free(&Pot_mpi);
      MPI_Type_free(&Rot_mpi);
      MPI_Type_free(&Side_mpi);
      MPI_Type_free(&Back_mpi);
